...
2. **Anything else** → `forward_request` (passthrough; strips a `/v1` or `/v4` client-side version prefix from the path, then forwards to `<api_base>/...`. GLM's `api_base` ends in `/v4` because that's GLM's upstream API version — independent of the client-side prefix).
